...
class Dialog(QtWidgets.QDialog):
    def __init__(self):
        ...
        # Middle column
        self.middleColumn = MiddleColumn(self)
        self.layout.addLayout(self.middleColumn)
        # Right column
        ...
        self.middleColumn.bottomPanel.inputLine.setFocus()
...
class MiddleColumn(QtWidgets.QVBoxLayout):
    ...
    class TopPanel(QtWidgets.QFrame):
        def __init__(self, owner):
            ...
            layout.addWidget(self.PaneTabMenu(owner.owner), 2, 1)
            layout.setRowMinimumHeight(2, 20)
            # Tab type menu
            layout.addWidget(QtWidgets.QLabel("Tab type:"), 3, 0)
            layout.addWidget(self.PaneTabTypeMenu(owner.owner), 3, 1)
            layout.setRowMinimumHeight(3, 20)
            # Autosave
            layout.addWidget(self.AutoSaveCheckBox(owner.owner), 4, 0)
            layout.setRowMinimumHeight(4, 20)
            # Pin tab
            layout.addWidget(self.PinCheckBox(owner.owner), 5, 0)
